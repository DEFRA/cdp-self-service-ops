...
describe('deploy-service-payload', () => {
  ...
  test('payload only accepts valid looking versions', () => {
    const payload = { imageName: 'foo', version: 'v1.0.0', cluster: 'frontend' }
    expect(Joi.assert(payload, schema)).toBeUndefined()
  })

  test('payload fails on invalid versions', () => {
    const payload = { imageName: 'foo', version: 'latest', cluster: 'frontend' }
  ...
  test('payload only accepts valid clusters', () => {
    expect(
      Joi.assert(
        { imageName: 'foo', version: 'v1.0.0', cluster: 'frontend' },
        schema
      )
    ).toBeUndefined()
    expect(
      Joi.assert(
        { imageName: 'foo', version: 'v1.0.0', cluster: 'backend' },
        schema
      )
    ).toBeUndefined()
  })
